...
with open(file, 'rb') as f:
  ...
  p    = np.loadtxt(file,skiprows=3, usecols=(5,))
...
pw = griddata((x, y), p, (xi[None,:], yi[:,None]), method='nearest')
pi = griddata((x, y), p, (xi[None,:], yi[:,None]), method='linear')
uui = griddata((x, y), uu, (xi[None,:], yi[:,None]), method='linear')
vvi = griddata((x, y), vv, (xi[None,:], yi[:,None]), method='linear')
wwi = griddata((x, y), ww, (xi[None,:], yi[:,None]), method='linear')
uvi = griddata((x, y), uv, (xi[None,:], yi[:,None]), method='linear')

for i in range (0,len(xi)):
  for j in range (0,len(yi)):
    if ( yi[j] < hill(xi[i])): ui[j,i] = 0. #np.nan
    if ( yi[j] < hill(xi[i])): vi[j,i] = 0. #np.nan
    if ( yi[j] < hill(xi[i])): pi[j,i] = pw[j,i] #np.nan
    if ( yi[j] < hill(xi[i])): uui[j,i] = 0. #np.nan
    if ( yi[j] < hill(xi[i])): vvi[j,i] = 0. #np.nan
    if ( yi[j] < hill(xi[i])): wwi[j,i] = 0. #np.nan
    if ( yi[j] < hill(xi[i])): uvi[j,i] = 0. #np.nan


fig, ax1 = plt.subplots(figsize=(10,3))
plt.contour(xi,yi,pi,15,linewidths=0.5,colors='k')
cntr1 = ax1.contourf(xi, yi, pi, 15, cmap="jet")
...
# Interpolate p
f = interpolate.interp2d(xi, yi, pi, kind='linear',fill_value=0.)
for i in range (0,len(xnew)):
  pnew = np.concatenate([pnew , f(xnew[i], ynew[i])])
  
# Interpolate uu
f = interpolate.interp2d(xi, yi, uui, kind='linear',fill_value=0.)
for i in range (0,len(xnew)):
  uunew = np.concatenate([uunew , f(xnew[i], ynew[i])])
  
# Interpolate vv
f = interpolate.interp2d(xi, yi, vvi, kind='linear',fill_value=0.)
for i in range (0,len(xnew)):
  vvnew = np.concatenate([vvnew , f(xnew[i], ynew[i])])

# Interpolate ww
f = interpolate.interp2d(xi, yi, wwi, kind='linear',fill_value=0.)
for i in range (0,len(xnew)):
  wwnew = np.concatenate([wwnew , f(xnew[i], ynew[i])])
  
# Interpolate uv
f = interpolate.interp2d(xi, yi, uvi, kind='linear',fill_value=0.)
for i in range (0,len(xnew)):
  uvnew = np.concatenate([uvnew , f(xnew[i], ynew[i])])
...
# Perform linear interpolation of the data (x,y) on a grid defined by (xi,yi)
uii = griddata((xnew, ynew), pnew, (xi[None,:], yi[:,None]), method='linear')
...
tecplot_file = open("./tecplot_interp_ref_sol_mesh"+mesh+"_Re"+Re+"-P.dat","w")
tecplot_file.write(str(nnodes)+'\n')
for i in range(len(xnew)):
...
tecplot_file = open("./tecplot_interp_ref_sol_mesh"+mesh+"_Re"+Re+"-UU.dat","w")
tecplot_file.write(str(nnodes)+'\n')
for i in range(len(xnew)):
...
tecplot_file = open("./tecplot_interp_ref_sol_mesh"+mesh+"_Re"+Re+"-VV.dat","w")
tecplot_file.write(str(nnodes)+'\n')
for i in range(len(xnew)):
...
tecplot_file = open("./tecplot_interp_ref_sol_mesh"+mesh+"_Re"+Re+"-WW.dat","w")
tecplot_file.write(str(nnodes)+'\n')
for i in range(len(xnew)):
...
tecplot_file = open("./tecplot_interp_ref_sol_mesh"+mesh+"_Re"+Re+"-UV.dat","w")
tecplot_file.write(str(nnodes)+'\n')
for i in range(len(xnew)):
